...
class DatabaseWriterActorTest extends ActorSpec with BeforeAndAfterEach {
  ...
  describe("DatabaseWriterActor") {
    describe("using sqlserver") {
      describe("initialise") {
        it("should create the tables", DbTest, DbTestSqlServer) {
          ...
          val results = statement.executeQuery(
            s"SELECT UPPER(TABLE_NAME) AS TABLE_NAME FROM INFORMATION_SCHEMA.TABLES WHERE UPPER(TABLE_CATALOG) = UPPER('$databaseName') AND UPPER(TABLE_NAME) = UPPER('ACCOUNTS')"
          )
        ...
        it("should not create tables that are already existing", DbTest, DbTestSqlServer) {
          val connection = java.sql.DriverManager.getConnection(
            s"jdbc:sqlserver://$databaseHost:$databasePort;databasename=$databaseName",
            databaseUser,
            databasePass
        ...
        it("should create primary keys if defined", DbTest, DbTestSqlServer) {
          val connection = java.sql.DriverManager.getConnection(
            s"jdbc:sqlserver://$databaseHost:$databasePort;databasename=$databaseName",
            databaseUser,
            databasePass
          ...
          an[SQLServerException] should be thrownBy statement.execute(
            "INSERT INTO ACCOUNTS VALUES(1, 'Jane Doe', NULL, '2001-01-02', 2.76)"
          )
          connection.close()
        }

        it("should create auto-increment columns if defined", DbTest, DbTestSqlServer) {
          val connection = java.sql.DriverManager.getConnection(
            s"jdbc:sqlserver://$databaseHost:$databasePort;databasename=$databaseName",
            databaseUser,
            databasePass
        ...
        it("should fail to create foreign keys without primary keys", DbTest, DbTestSqlServer) {
          val connection = java.sql.DriverManager.getConnection(
            s"jdbc:sqlserver://$databaseHost:$databasePort;databasename=$databaseName",
            databaseUser,
            databasePass
          ...
          writer.stateName should be(BaseWriter.State.Initializing)
          EventFilter[SQLServerException](source = writer.path.toString, occurrences = 1) intercept {
            writer ! BaseWriterMessages.InitializeTarget
            writer ! AreYouReady
            val expectedMsg = ReadyToWork
            expectMsg(expectedMsg)
        ...
        it("should create the unique columns if defined", DbTest, DbTestSqlServer) {
          val connection = java.sql.DriverManager.getConnection(
            s"jdbc:sqlserver://$databaseHost:$databasePort;databasename=$databaseName",
            databaseUser,
            databasePass
        ...
        it("should create primary keys, foreign keys and auto increments", DbTest, DbTestSqlServer) {
          val connection = java.sql.DriverManager.getConnection(
            s"jdbc:sqlserver://$databaseHost:$databasePort;databasename=$databaseName",
            databaseUser,
            databasePass
          ...
          val statement = connection.createStatement()
          val results = statement.executeQuery(
            s"SELECT UPPER(TABLE_NAME) AS TABLE_NAME FROM INFORMATION_SCHEMA.TABLES WHERE UPPER(TABLE_CATALOG) = UPPER('$databaseName') AND (UPPER(TABLE_NAME) = UPPER('ACCOUNTS') OR UPPER(TABLE_NAME) = 'COMPANIES') ORDER BY TABLE_NAME ASC"
          )
      ...
      describe("writing data") {
        describe("using a single sequence") {
          describe("when given data for a single row") {
            it("should write a sequence row", DbTest, DbTestSqlServer) {
              val connection = java.sql.DriverManager.getConnection(
                s"jdbc:sqlserver://$databaseHost:$databasePort;databasename=$databaseName",
                databaseUser,
                databasePass
          ...
          describe("when given data for multiple rows") {
            describe("without primary key") {
              it("should write all possible sequence rows", DbTest, DbTestSqlServer) {
                val connection = java.sql.DriverManager.getConnection(
                  s"jdbc:sqlserver://$databaseHost:$databasePort;databasename=$databaseName",
                  databaseUser,
                  databasePass
            ...
            describe("with primary key") {
              it("should write new and update existing rows", DbTest, DbTestSqlServer) {
                val connection = java.sql.DriverManager.getConnection(
                  s"jdbc:sqlserver://$databaseHost:$databasePort;databasename=$databaseName",
                  databaseUser,
                  databasePass
        ...
        describe("when retrieving ordered column data") {
          it("should write the columns in correct order", DbTest, DbTestSqlServer) {
            val connection = java.sql.DriverManager.getConnection(
              s"jdbc:sqlserver://$databaseHost:$databasePort;databasename=$databaseName",
              databaseUser,
              databasePass
      ...
      describe("using auto increment columns") {
        it("should collect the written auto increment values", DbTest, DbTestSqlServer) {
          val connection = java.sql.DriverManager.getConnection(
            s"jdbc:sqlserver://$databaseHost:$databasePort;databasename=$databaseName",
            databaseUser,
            databasePass
      ...
      describe("using NULL values") {
        it("should set the correct parameter columns to null", DbTest, DbTestSqlServer) {
          val connection = java.sql.DriverManager.getConnection(
            s"jdbc:sqlserver://$databaseHost:$databasePort;databasename=$databaseName",
            databaseUser,
            databasePass
