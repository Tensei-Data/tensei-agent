...
class SchemaExtractionTest extends ActorSpec with BeforeAndAfterEach {
  ...
  val expectedDfasdlContent =
    """
      ...
      |    <elem id="wages_row">
      |      <num db-column-name="employee_id" db-foreign-key="employees_row_id" id="wages_row_employee_id" max-digits="20"/>
      |      <num db-column-name="salary_group" db-foreign-key="salary_groups_row_id" id="wages_row_salary_group" max-digits="11"/>
  ...
  describe("Database schema extraction") {
    describe("using mariadb") {
      it("should extract the correct schema", DbTest, DbTestMariaDb) {
