...
class NormalisationTest extends ActorSpec with BeforeAndAfterEach {
  ...
  override protected def beforeEach(): Unit = {
    ...
    createSourceData(connection)
    createTargetStructure(connection)
    connection.close()
    super.beforeEach()
  }
  ...
  private def createSourceData(connection: java.sql.Connection): Unit = {
    val s = connection.createStatement()
    s.execute("""
  ...
  private def createTargetStructure(connection: java.sql.Connection): Unit = {
    val s = connection.createStatement()
    s.execute("""
    ...
    s.execute("""
        ...
        |  lastname VARCHAR(254),
        |  birthday DATE,
        |  century_id INTEGER REFERENCES T_CENTURIES (id)
        |)
      """.stripMargin)
    s.execute("""
        |CREATE TABLE t_salary (
        |  employee_id INTEGER REFERENCES T_EMPLOYEES (id),
        |  amount DECIMAL(10,2)
        |)
      """.stripMargin)
    s.close()
    ...
    describe("using sqlite") {
      describe("using one to one mappings") {
        describe("with single mappings") {
          ...
          it("should consider existing unique values", DbTest, DbTestSqlite) {
            val sqliteFile = SQLITE_FILE.getAbsolutePath.replace("\\", "/")
            val connection = java.sql.DriverManager.getConnection(s"jdbc:sqlite:$sqliteFile")
