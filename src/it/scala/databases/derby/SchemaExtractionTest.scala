...
class SchemaExtractionTest extends ActorSpec with BeforeAndAfterEach {
  ...
  val databasePath: Path = {
    val d = Files.createTempDirectory("Derby-SchemaExtraction-Test")
    d.toAbsolutePath
  }
  ...
  override protected def beforeEach(): Unit = {
    withClue(
      ...
      val connection =
        java.sql.DriverManager.getConnection(s"jdbc:derby:$databasePath/$databaseName;create=true")
  ...
  describe("Database schema extraction") {
    describe("using Derby") {
      describe("with filesystem") {
        it("should extract the correct schema without keys", DbTest, DbTestDerby) {
          val connection = withClue("Database should have been created before the test!")(
            java.sql.DriverManager.getConnection(s"jdbc:derby:$databasePath/$databaseName")
          )
          ...
          response.result match {
            ...
            case \/-(success) =>
              success.content.trim should be(expectedDfasdlContentWithoutKeys.trim)
          }
        }
      }

      describe("with in-memory") {
        ignore("should extract the correct schema") {
          val connection =
            java.sql.DriverManager.getConnection(s"jdbc:derby:memory:test;create=true")
          createDatabaseTables(connection)
          val src = ConnectionInformation(
            uri = new java.net.URI(connection.getMetaData.getURL),
            dfasdlRef = None
