    ...
    describe("using hsqldb") {
      describe("using one to one mappings") {
        describe("with single mappings") {
          it("should replace changed auto-increment values", DbTest, DbTestHsqlDb) {
            ...
            withClue("Written data should be correct!") {
              ...
              if (r.next()) {
                r.getBigDecimal("amount").compareTo(expectedData(r.getString("name"))) should be(0)

                while (r.next()) {
                  r.getBigDecimal("amount").compareTo(expectedData(r.getString("name"))) should be(
                    0
                  )
                }
              } else
