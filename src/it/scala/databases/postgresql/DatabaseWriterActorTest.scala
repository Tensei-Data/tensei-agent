...
class DatabaseWriterActorTest extends ActorSpecWithDebugLog with BeforeAndAfterEach {
  ...
  describe("DatabaseWriterActor") {
    describe("using postgresql") {
      describe("initialise") {
        it("should create the tables", DbTest, DbTestPostgresql) {
          val connection = java.sql.DriverManager.getConnection(
          ...
          val results = statement.executeQuery(
            s"SELECT UPPER(TABLENAME) AS TABLENAME FROM PG_TABLES WHERE UPPER(SCHEMANAME) = 'PUBLIC' AND UPPER(TABLENAME) = 'ACCOUNTS' ORDER BY TABLENAME ASC"
          )
        ...
        it("should not create tables that are already existing", DbTest, DbTestPostgresql) {
          val connection = java.sql.DriverManager.getConnection(
            s"jdbc:postgresql://$databaseHost:$databasePort/$databaseName",
            databaseUser,
            databasePass
        ...
        it("should create primary keys if defined", DbTest, DbTestPostgresql) {
          val connection = java.sql.DriverManager.getConnection(
            s"jdbc:postgresql://$databaseHost:$databasePort/$databaseName",
            databaseUser,
            databasePass
          ...
          val statement = connection.createStatement()
          val results = statement.executeQuery(
            s"SELECT UPPER(TABLENAME) AS TABLENAME, TABLEOWNER FROM PG_TABLES WHERE UPPER(SCHEMANAME) = 'PUBLIC' AND UPPER(TABLENAME) = 'ACCOUNTS' ORDER BY TABLENAME ASC"
          )
          withClue("Database table 'accounts' should be created!'") {
            results.next() should be(true)
            results.getString("TABLENAME") shouldEqual "ACCOUNTS"
            withClue("Table ownership should be correct!")(
              results.getString("TABLEOWNER") shouldEqual databaseUser
            )
          }
          statement.execute("INSERT INTO ACCOUNTS VALUES(1, 'John Doe', NULL, '2001-01-01', 3.14)")
          an[PSQLException] should be thrownBy statement.execute(
            "INSERT INTO ACCOUNTS VALUES(1, 'Jane Doe', NULL, '2001-01-02', 2.76)"
          )
          connection.close()
        }

        it("should create auto-increment columns if defined", DbTest, DbTestPostgresql) {
          val connection = java.sql.DriverManager.getConnection(
            s"jdbc:postgresql://$databaseHost:$databasePort/$databaseName",
            databaseUser,
            databasePass
        ...
        it("should fail to create foreign keys without primary keys", DbTest, DbTestPostgresql) {
          val connection = java.sql.DriverManager.getConnection(
            s"jdbc:postgresql://$databaseHost:$databasePort/$databaseName",
            databaseUser,
            databasePass
          ...
          writer.stateName should be(BaseWriter.State.Initializing)
          EventFilter[PSQLException](source = writer.path.toString, occurrences = 1) intercept {
            writer ! BaseWriterMessages.InitializeTarget
            writer ! AreYouReady
            val expectedMsg = ReadyToWork
            expectMsg(expectedMsg)
        ...
        it("should create the unique columns if defined", DbTest, DbTestPostgresql) {
          val connection = java.sql.DriverManager.getConnection(
            s"jdbc:postgresql://$databaseHost:$databasePort/$databaseName",
            databaseUser,
            databasePass
        ...
        it("should create primary keys, foreign keys and auto increments", DbTest, DbTestPostgresql) {
          val connection = java.sql.DriverManager.getConnection(
            s"jdbc:postgresql://$databaseHost:$databasePort/$databaseName",
            databaseUser,
            databasePass
          ...
          val statement = connection.createStatement()
          val results = statement.executeQuery(
            s"SELECT UPPER(TABLENAME) AS TABLENAME FROM PG_TABLES WHERE UPPER(SCHEMANAME) = 'PUBLIC' AND (UPPER(TABLENAME) = 'ACCOUNTS' OR UPPER(TABLENAME) = 'COMPANIES') ORDER BY TABLENAME ASC"
          )
      ...
      describe("writing data") {
        describe("using a single sequence") {
          describe("when given data for a single row") {
            it("should write a sequence row", DbTest, DbTestPostgresql) {
              val connection = java.sql.DriverManager.getConnection(
                s"jdbc:postgresql://$databaseHost:$databasePort/$databaseName",
                databaseUser,
                databasePass
          ...
          describe("when given data for multiple rows") {
            describe("without primary key") {
              it("should write all possible sequence rows", DbTest, DbTestPostgresql) {
                val connection = java.sql.DriverManager.getConnection(
                  s"jdbc:postgresql://$databaseHost:$databasePort/$databaseName",
                  databaseUser,
                  databasePass
            ...
            describe("with primary key") {
              it("should write new and update existing rows", DbTest, DbTestPostgresql) {
                val connection = java.sql.DriverManager.getConnection(
                  s"jdbc:postgresql://$databaseHost:$databasePort/$databaseName",
                  databaseUser,
                  databasePass
        ...
        describe("when retrieving ordered column data") {
          it("should write the columns in correct order", DbTest, DbTestPostgresql) {
            val connection = java.sql.DriverManager.getConnection(
              s"jdbc:postgresql://$databaseHost:$databasePort/$databaseName",
              databaseUser,
              databasePass
      ...
      describe("using auto increment columns") {
        it("should collect the written auto increment values", DbTest, DbTestPostgresql) {
          val connection = java.sql.DriverManager.getConnection(
            s"jdbc:postgresql://$databaseHost:$databasePort/$databaseName",
            databaseUser,
            databasePass
      ...
      describe("using NULL values") {
        it("should set the correct parameter columns to null", DbTest, DbTestPostgresql) {
          val connection = java.sql.DriverManager.getConnection(
            s"jdbc:postgresql://$databaseHost:$databasePort/$databaseName",
            databaseUser,
            databasePass
