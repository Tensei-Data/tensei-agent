...
class NormalisationTest extends ActorSpec with BeforeAndAfterEach {
  ...
  private def createSourceData(): Unit = {
    val c = java.sql.DriverManager.getConnection(s"jdbc:h2:mem:$databaseName")
    ...
    describe("using h2") {
      describe("using one to one mappings") {
        describe("with single mappings") {
          it("should replace changed auto-increment values", DbTest, DbTestH2) {
            val connection = java.sql.DriverManager.getConnection(s"jdbc:h2:mem:$databaseName")
          ...
          it("should consider existing unique values", DbTest, DbTestH2) {
            val connection = java.sql.DriverManager.getConnection(s"jdbc:h2:mem:$databaseName")
