...
class SchemaExtractionTest extends ActorSpec with BeforeAndAfterEach {
  ...
  val databasePath: Path = {
    val d = Files.createTempDirectory("H2-SchemaExtraction-Test")
    d.toAbsolutePath
  }
  ...
  override protected def beforeEach(): Unit = {
    withClue(
      ...
      val connection = java.sql.DriverManager.getConnection(s"jdbc:h2:$databasePath/$databaseName")
  ...
  describe("Database schema extraction") {
    describe("using H2") {
      describe("with filesystem") {
        it("should extract the correct schema", DbTest, DbTestH2) {
          val connection = withClue("Database should have been created before the test!")(
            java.sql.DriverManager
              .getConnection(s"jdbc:h2:$databasePath/$databaseName;IFEXISTS=TRUE")
          )
