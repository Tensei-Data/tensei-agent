...
class DatabaseWriterActorTest extends ActorSpec with BeforeAndAfterEach {
  ...
  describe("DatabaseWriterActor") {
    describe("using firebird") {
      describe("initialise") {
        it("should create the tables", DbTest, DbTestFirebird) {
          ...
          withClue("Database table 'accounts' should be created!'") {
            ...
            results.getString("TABLENAME").trim shouldEqual "ACCOUNTS"
          }
          connection.close()
        }

        it("should not create tables that are already existing", DbTest, DbTestFirebird) {
          val connection = java.sql.DriverManager.getConnection(
            s"jdbc:firebirdsql://$databaseHost:$databasePort//$databasePath/$databaseName",
            databaseUser,
            databasePass
        ...
        it("should create primary keys if defined", DbTest, DbTestFirebird) {
          val connection = java.sql.DriverManager.getConnection(
            s"jdbc:firebirdsql://$databaseHost:$databasePort//$databasePath/$databaseName",
            databaseUser,
            databasePass
          ...
          val statement = connection.createStatement()
          val results = statement.executeQuery(
            s"SELECT rdb$$relation_name AS TABLENAME FROM rdb$$relations WHERE rdb$$view_blr IS NULL AND UPPER(rdb$$relation_name) = 'ACCOUNTS' AND (rdb$$system_flag IS NULL OR rdb$$system_flag = 0) ORDER BY TABLENAME ASC"
          )
          withClue("Database table 'accounts' should be created!'") {
            results.next() should be(true)
            results.getString("TABLENAME").trim shouldEqual "ACCOUNTS"
          ...
          an[org.firebirdsql.jdbc.FBSQLException] should be thrownBy statement.execute(
            "INSERT INTO ACCOUNTS VALUES(1, 'Jane Doe', NULL, '2001-01-02', 2.76)"
          )
          connection.close()
        }

        it("should create auto-increment columns if defined", DbTest, DbTestFirebird) {
          val connection = java.sql.DriverManager.getConnection(
            s"jdbc:firebirdsql://$databaseHost:$databasePort//$databasePath/$databaseName",
            databaseUser,
            databasePass
        ...
        it("should fail to create foreign keys without primary keys", DbTest, DbTestFirebird) {
          val connection = java.sql.DriverManager.getConnection(
            s"jdbc:firebirdsql://$databaseHost:$databasePort//$databasePath/$databaseName",
            databaseUser,
            databasePass
          ...
          writer.stateName should be(BaseWriter.State.Initializing)
          EventFilter[org.firebirdsql.jdbc.FBSQLException](source = writer.path.toString,
        ...
        it("should create the unique columns if defined", DbTest, DbTestFirebird) {
          val connection = java.sql.DriverManager.getConnection(
            s"jdbc:firebirdsql://$databaseHost:$databasePort//$databasePath/$databaseName",
            databaseUser,
            databasePass
        ...
        it("should create primary keys, foreign keys and auto increments", DbTest, DbTestFirebird) {
          val connection = java.sql.DriverManager.getConnection(
            s"jdbc:firebirdsql://$databaseHost:$databasePort//$databasePath/$databaseName",
            databaseUser,
            databasePass
          ...
          val statement = connection.createStatement()
          val results = statement.executeQuery(
            s"SELECT rdb$$relation_name AS TABLENAME FROM rdb$$relations WHERE rdb$$view_blr IS NULL AND UPPER(rdb$$relation_name) = 'ACCOUNTS' OR UPPER(rdb$$relation_name) = 'COMPANIES' AND (rdb$$system_flag IS NULL OR rdb$$system_flag = 0) ORDER BY TABLENAME ASC"
          )
          withClue("Database tables should be created!'") {
            results.next() should be(true)
            results.getString("TABLENAME").trim shouldEqual "ACCOUNTS"
            results.next() should be(true)
            results.getString("TABLENAME").trim shouldEqual "COMPANIES"
          }
      ...
      describe("writing data") {
        describe("using a single sequence") {
          describe("when given data for a single row") {
            it("should write a sequence row", DbTest, DbTestFirebird) {
              val connection = java.sql.DriverManager.getConnection(
                s"jdbc:firebirdsql://$databaseHost:$databasePort//$databasePath/$databaseName",
                databaseUser,
                databasePass
          ...
          describe("when given data for multiple rows") {
            describe("without primary key") {
              it("should write all possible sequence rows", DbTest, DbTestFirebird) {
                val connection = java.sql.DriverManager.getConnection(
                  s"jdbc:firebirdsql://$databaseHost:$databasePort//$databasePath/$databaseName",
                  databaseUser,
                  databasePass
                ...
                val statement = connection.createStatement()
                val results   = statement.executeQuery("SELECT * FROM ACCOUNTS ORDER BY id ASC")
                withClue("Data should have been written to the database!") {
                  results.next() should be(true)
                  results.getLong("id") should be(1)
                  results.getString("name") should be("Max Mustermann")
            ...
            describe("with primary key") {
              it("should write new and update existing rows", DbTest, DbTestFirebird) {
                val connection = java.sql.DriverManager.getConnection(
                  s"jdbc:firebirdsql://$databaseHost:$databasePort//$databasePath/$databaseName",
                  databaseUser,
                  databasePass
        ...
        describe("when retrieving ordered column data") {
          it("should write the columns in correct order", DbTest, DbTestFirebird) {
            val connection = java.sql.DriverManager.getConnection(
              s"jdbc:firebirdsql://$databaseHost:$databasePort//$databasePath/$databaseName",
              databaseUser,
              databasePass
      ...
      // FIXME: Dieser Test läuft nicht korrekt!
      describe("using auto increment columns") {
        it("should collect the written auto increment values", DbTest, DbTestFirebird) {
          val connection = java.sql.DriverManager.getConnection(
            s"jdbc:firebirdsql://$databaseHost:$databasePort//$databasePath/$databaseName",
            databaseUser,
            databasePass
      ...
      describe("using NULL values") {
        it("should set the correct parameter columns to null", DbTest, DbTestFirebird) {
          val connection = java.sql.DriverManager.getConnection(
            s"jdbc:firebirdsql://$databaseHost:$databasePort//$databasePath/$databaseName",
            databaseUser,
            databasePass
