...
object ParserDataContainerBigDecimalMemoryUsage {
  ...
  def main(args: Array[String]): Unit = {
    ...
    println(
      s"Going to create a Vector with $SAMPLE_SIZE ParserDataContainer elements using java.math.BigDecimal."
    )
    println(
    ...
    for (cnt <- 1L to SAMPLE_SIZE) {
      val d = scala.util.Random.nextDouble() + scala.util.Random.nextLong().toDouble
      builder += ParserDataContainer(
        data = java.math.BigDecimal.valueOf(d),
        elementId = "AN-ID",
