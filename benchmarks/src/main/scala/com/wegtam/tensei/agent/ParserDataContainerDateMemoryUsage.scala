...
object ParserDataContainerDateMemoryUsage {

  def main(args: Array[String]): Unit = {
    val meter   = new MemoryMeter()
    val hasher  = LongHashFunction.xx()
    val builder = Vector.newBuilder[ParserDataContainer]
    println(s"Going to create a Vector with $SAMPLE_SIZE ParserDataContainer elements using Date.")
    println(
    ...
    for (cnt <- 1L to SAMPLE_SIZE) {
      builder += ParserDataContainer(
        data = java.sql.Date.valueOf(baseDate.plusDays(cnt)),
        elementId = "AN-ID",
