...
object ParserDataContainerStringMemoryUsage {
  ...
  def main(args: Array[String]): Unit = {
    ...
    println(
      s"Going to create a Vector with $SAMPLE_SIZE ParserDataContainer elements using strings of $stringSize chars."
    )
    println(
    ...
    for (cnt <- 1L to SAMPLE_SIZE) {
      builder += ParserDataContainer(
        data = scala.util.Random.alphanumeric.take(stringSize).mkString,
        elementId = "AN-ID",
